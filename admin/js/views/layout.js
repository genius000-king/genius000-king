...
import { fld, toggle, select, slider, color } from '../ui/fields.js';
...
async function ensureRows() {
  ...
  for (let i = 0; i < missing.length; i++) {
    const row = await insert('layout', {
      section_key: missing[i][0], sort: get('layout').length + i + 1,
      columns: 2, align: 'stretch', gap: '', bg_type: 'none', bg_value: '', visible: true,
    }).catch(() => null);
...
export async function render(host) {
  ...
  const draw = () => {
    ...
    list.replaceChildren(...rows.map((r, i) => el('div', {
      class: `sort-row ${r.visible === false ? 'is-off' : ''}`, 'data-id': r.id,
    }, [
      ...
      el('span', { class: 'fld__hint nowrap' }, [`${r.columns || 2} أعمدة`]),
      toggle(r.visible !== false, (v) => {
        r.visible = v;
        save('layout', r.id, { visible: v });
        draw();
        reloadPreview();
      }),
    ])));
    ...
    makeSortable(list, (ids) => {
      ids.forEach((id, i) => save('layout', id, { sort: i + 1 }));
      setAll('layout', get('layout').map((l) =>
        ids.includes(l.id) ? { ...l, sort: ids.indexOf(l.id) + 1 } : l));
      draw();
      reloadPreview();
    ...
    wrap.replaceChildren(el('div', { class: 'view' }, [
      el('div', { class: 'view__head' }, [
        el('div', {}, [
          el('h1', { class: 'view__title' }, ['ترتيب الأقسام']),
          el('p', { class: 'view__sub' }, ['اسحب لتغيير ترتيب ظهور الأقسام في الصفحة']),
        ]),
      ]),
      list,
    ]));
